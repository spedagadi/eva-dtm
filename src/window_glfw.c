...
static void release_current(void *priv) { glfwMakeContextCurrent(NULL); }
#endif
...
static struct window *glfw_create(pl_log log,
                                  const struct window_params *params) {
  ...
  if (!glfwVulkanSupported()) {
    fprintf(stderr,
            "GLFW: No vulkan support! Perhaps recompile with -DUSE_GL\n");
    goto error;
  }
  ...
  for (int i = 0; i < PL_ARRAY_SIZE(gl_vers); i++) {
    printf("gl_vers[i].api %d gl_vers[i].major %d gl_vers[i].minor %d "
           "gl_vers[i].profile %d\n",
           gl_vers[i].api, gl_vers[i].major, gl_vers[i].minor,
           gl_vers[i].profile);
    ...
    p->win = glfwCreateWindow(params->width, params->height, params->title,
                              NULL, NULL);
  ...
  uint32_t num;
  p->vk_inst = pl_vk_inst_create(
      log,
      pl_vk_inst_params(.get_proc_addr = get_vk_proc_addr, .debug = DEBUG,
  ...
  p->vk = pl_vulkan_create(
      log, pl_vulkan_params(.instance = p->vk_inst->instance,
                            .get_proc_addr = p->vk_inst->get_proc_addr,
                            .surface = p->surf, .allow_software = true, ));
  if (!p->vk) {
    fprintf(stderr, "libplacebo: Failed creating vulkan device\n");
    goto error;
  }

  p->w.swapchain = pl_vulkan_create_swapchain(
      p->vk,
      pl_vulkan_swapchain_params(.surface = p->surf,
...
#ifdef USE_GL
  p->gl = pl_opengl_create(
      log,
      pl_opengl_params(.allow_software = true, .debug = false,
#ifdef HAVE_EGL
                       ...
                       .get_proc_addr = glfwGetProcAddress, .priv = p->win, ));
  if (!p->gl) {
    fprintf(stderr, "libplacebo: Failed creating opengl device\n");
    goto error;
  }

  p->w.swapchain = pl_opengl_create_swapchain(
      p->gl, pl_opengl_swapchain_params(.swap_buffers =
                                            (void (*)(void *))glfwSwapBuffers,
                                        .priv = p->win, ));
  ...
  p->w.swapchain = pl_d3d11_create_swapchain(
      p->d3d11,
      pl_d3d11_swapchain_params(.window = glfwGetWin32Window(p->win), ));
  if (!p->w.swapchain) {
...
static void glfw_destroy(struct window **window) {
  ...
  if (p->surf) {
    PFN_vkDestroySurfaceKHR vkDestroySurfaceKHR =
        (PFN_vkDestroySurfaceKHR)p->vk_inst->get_proc_addr(
            p->vk_inst->instance, "vkDestroySurfaceKHR");
...
static bool glfw_toggle_fullscreen(const struct window *window,
                                   bool fullscreen) {
  struct priv *p = (struct priv *)window;
...
static void glfw_get_frame(const struct window *window, int width, int height,
                           void *dst_data) {
  struct priv *p = (struct priv *)window;
  glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, dst_data);
}
