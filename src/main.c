    ...
    while (!p->exit_thread) {
        ...
        while ((ret = avcodec_receive_frame(p->codec, frame)) == 0) {
            ...
            if (p->inv_fps_acc >= p->inv_fps_target) {
                ...
                //printf("Frame sent to renderer {%lu:%f}\n", num_frames, p->inv_fps_acc);
                //if (!is_avq_full(p->decoder_queue))
                //    avq_enqueue(p->decoder_queue, frame);
                //else
                //    fprintf(stderr, "Decoder queue is full, dropping frame with pts: %ld\n",
                //frame->pts);
                ...
                if(queue_count < 10)
                {
                    ...
                    //    fprintf(stderr, "Render queue is full, dropping frame with pts: %ld\n",
                    //            frame->pts);
                    av_frame_free(&frame);
                }
                p->inv_fps_acc -= p->inv_fps_target;
            } else {
                //printf("Frame dropped {%d:%f}\n", p->num_frames, p->inv_fps_acc);
                av_frame_free(&frame);
            }
...
enum pl_queue_status get_decoded_frame(struct pl_source_frame *out_frame,
    ...
    out_frame->pts = new_frame->pts,
    out_frame->duration = state.frame_duration,
    out_frame->map = map_frame,
    out_frame->unmap = unmap_frame,
    out_frame->discard = discard_frame,
    out_frame->frame_data = new_frame,
    ...
    while (!p->win->window_lost) {
        ...
        if (pts_target) {
            ...
            if (pts_target >= pts_now) {
                ...
                fprintf(stderr, "Missed PTS target %.3f (%.3f ms in the past) Queued frames : {%d}\n",
